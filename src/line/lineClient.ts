...
export async function replyToLine(replyToken: string, text: string, quickReplies?: QuickReplies) {
  ...
  try {
    const resp = await axios.post("https://api.line.me/v2/bot/message/reply", { replyToken, messages: [message] }, { headers: { Authorization: `Bearer ${TOKEN}`, "Content-Type": "application/json" }, timeout: 7000 });
...
export async function pushToLine(userId: string, text: string, quickReplies?: QuickReplies) {
  ...
  try {
    const resp = await axios.post("https://api.line.me/v2/bot/message/push", { to: userId, messages: [message] }, { headers: { Authorization: `Bearer ${TOKEN}`, "Content-Type": "application/json" }, timeout: 7000 });
