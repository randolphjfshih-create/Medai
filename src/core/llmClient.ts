  ...
  for (let i = 0; i <= maxRetries; i++) {
    try {
      ...
      const completion = await client.chat.completions.create(
        { model: MODEL, messages: [ { role: "system", content: systemInstruction }, { role: "user", content: userContext } ], temperature: 0.4, max_tokens: 180 },
      ...
      lastErr = err; const code = err?.status || err?.code;
